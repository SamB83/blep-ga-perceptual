function [splipmqIR,numZc,order] = getSplinePMIR(fstop,widtrans,deltaStop,deltaPass,wtPass,wtStop,wtTrans1,wtTrans2,tableSize,getplot)
  %
  %   getSplinePMIR Applies a Parks-Mclellan filter to the given input
  ...
  %   [splipmqIR,numZc,order] = getSplinePMIR(fstop,widtrans,deltaStop,
  ...
  %   Output :
  %   
  %   splipmqIR: sinc function returned.
 ...
 %   sbwt = deltap/deltas;
  ...
  % b(k)=b(n+2-k),     k=1,...,n+1 symmtery relation of coeffs.
   bcoeffnum = firpm(order,freqvec,ampvec,wtvec);
    ...
    [Ts,T] = ndgrid(newt,tn);
    splipmqIR = sinc(Ts - T)*hn;

    %find zero-crossings in the interpolated sinc:
    i=1:length(splipmqIR)-1;
    k=find ((splipmqIR(i)>0 & splipmqIR(i+1)<0) | (splipmqIR(i)<0 & splipmqIR(i+1)>0));
    numZc = length(k);

    if getplot
    figure()
    plot(tn,hn,'o',newt,splipmqIR,newt(k),splipmqIR(k),'x')
    grid on
    axis tight
    xlabel Time, ylabel Sinc
    title(['Optimal design with order - ',num2str(order),',zero crossings - ',num2str(numZc)])
